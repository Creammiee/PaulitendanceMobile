    ...
    useEffect(() => {
        ...
        const loadParentData = async () => {
            try {
                ...
                if (!parentSnap.exists() || !parentSnap.data()?.studentIds?.length) {
                    setLoading(false);
                    return;
                }

                const studentId = parentSnap.data().studentIds[0];

                // 2. Get Student Profile (for context)
                const studentSnap = await getDoc(doc(db, 'users', studentId));
                if (studentSnap.exists()) {
                    setStudentProfile(studentSnap.data() as UserProfile);
                }
        ...
        const unsubPromise = loadParentData();
        return () => {
    ...
    return (
        <View style={styles.container}>
            <View style={styles.header}>
                <Text style={styles.title}>Child's Attendance</Text>
                {studentProfile && <Text style={styles.studentName}>{studentProfile.fullName}</Text>}
            </View>
